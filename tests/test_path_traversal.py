"""
Path traversal attack tests comparing vulnerable (port 8765) and secure
(port 8766) MCP servers.  Payloads are loaded from exploits/payloads.json.
"""
...
# ---------------------------------------------------------------------------
# Vulnerable server — traversal payloads are NOT blocked
# ---------------------------------------------------------------------------

class TestVulnerableTraversal:
    """Vulnerable server has no path validation — no security blocks."""

    @pytest.fixture(autouse=True, scope="class")
    def _start(self, vuln_server):
        pass

    @pytest.mark.parametrize("name,path", TRAVERSAL)
    def test_traversal_not_blocked(self, name, path):
        result = asyncio.run(_call(VULN_URL, "read_secure_file", {"path": path}))
        text = _text(result)
        # Vulnerable server never sets isError — it has no security validation
        assert not result.isError, f"{name}: unexpected isError=True (blocked)"
        assert "traversal" not in text.lower(), f"{name}: unexpected traversal block"

    @pytest.mark.parametrize("name,path,reason", POISONING)
    def test_argument_poisoning_not_blocked(self, name, path, reason):
        args = {"path": path}
        if reason:
            args["reason"] = reason
        result = asyncio.run(_call(VULN_URL, "read_secure_file", args))
        assert not result.isError, f"{name}: unexpected isError=True (blocked)"

    def test_legitimate_hello_txt(self):
        result = asyncio.run(_call(VULN_URL, "read_secure_file", {"path": "hello.txt"}))
        text = _text(result)
        assert "[OK]" in text
